...
theorem add_right_cancel {a b c : R} (h : a + b = c + b) : a = c := by
  rw [← add_zero a, ← add_neg_cancel b, ← add_assoc, h, add_neg_cancel_right]
...
theorem neg_neg (a : R) : - -a = a := by
  rw [← add_zero (- -a), ← neg_add_cancel a, neg_add_cancel_left]
...
theorem mul_inv_cancel (a : G) : a * a⁻¹ = 1 := by
  ...
  rw [← one_mul (a * a⁻¹), ← inv_mul_cancel (a * a⁻¹), mul_assoc, h, inv_mul_cancel]
...
theorem mul_inv_rev (a b : G) : (a * b)⁻¹ = b⁻¹ * a⁻¹ := by
  ...
  rw [← mul_inv_cancel b]
  rw [← mul_assoc a, ← mul_assoc, ← mul_assoc, inv_mul_cancel, one_mul]
